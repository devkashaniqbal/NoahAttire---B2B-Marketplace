...
import { Building2, Loader2, Eye, EyeOff, ShoppingBag, Store } from 'lucide-react';
...
function RegisterContent() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(`Welcome to Noah Attire, ${newUser.name}!`);
      if (newUser.role === 'seller') router.push('/seller/onboarding');
      else if (newUser.role === 'buyer') router.push('/buyer/dashboard');
      else router.push('/');
    } catch (err) {
  ...
  const handleGoogle = async () => {
    ...
    try {
      const newUser = await googleLogin(form.role);
      toast.success(`Welcome to Noah Attire, ${newUser.name}!`);
      if (newUser.role === 'seller') router.push('/seller/onboarding');
      else if (newUser.role === 'buyer') router.push('/buyer/dashboard');
      else router.push('/');
    ...
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center bg-gray-50 py-12 px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">Join thousands of businesses on Noah Attire</p>
        </div>

        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-8">
          {/* Role Selection — admin role not exposed */}
          <div className="grid grid-cols-2 gap-3 mb-6">
            {[
              { role: 'buyer', label: 'Buyer', desc: 'Source products', icon: ShoppingBag },
              { role: 'seller', label: 'Seller', desc: 'List & sell', icon: Store },
